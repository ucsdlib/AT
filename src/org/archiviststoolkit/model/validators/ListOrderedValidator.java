...
public class ListOrderedValidator extends ATAbstractValidator {
	 ...
	 */
	public ListOrderedValidator(Accessions accession) {
		this.objectToValidate = accession;
	}

	public ListOrderedValidator() {
	}
	...
	public ValidationResult validate() {
		...
		ListOrdered modelToValidate = (ListOrdered)objectToValidate;

		ATPropertyValidationSupport support =
				new ATPropertyValidationSupport(modelToValidate, "List ordered");

		if (modelToValidate.getItems().size() == 0)
			support.addError("At least one list ordered item", "is mandatory");
