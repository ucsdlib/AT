...
public class ChronItemsValidator extends ATAbstractValidator {
	 ...
	 */
	public ChronItemsValidator(Accessions accession) {
		this.objectToValidate = accession;
	}

	public ChronItemsValidator() {
	}
	...
	public ValidationResult validate() {
		...
		ChronologyItems modelToValidate = (ChronologyItems)objectToValidate;

		ATPropertyValidationSupport support =
				new ATPropertyValidationSupport(modelToValidate, "Chronology Items");

		if (ValidationUtils.isBlank(modelToValidate.getEventDate()))
			support.addError("Date", "is mandatory");

		if (modelToValidate.getEvents().size() == 0)
			support.addError("At least one event", "is mandatory");
